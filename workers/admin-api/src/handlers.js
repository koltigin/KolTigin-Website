...
async function scriptCatalog(github) {
  const projectsJson = await readJson(github, "projects/projects.json", {});
  const categories = await readJson(github, "config/project-categories.json", []);
  return {
    allowed: getAllowedDownloadProjects(projectsJson),
    options: buildScriptOptions(projectsJson, categories)
  };
}
...
export async function handleScriptUpload(file, fields, github) {
  ...
  const { allowed } = await scriptCatalog(github);
  const project = resolveScriptProject(String((fields && fields.project) || ""), allowed);
  ...
  const path = scriptRepoPath(project, filename, allowed);
  ...
  return {
    ...
    folder,
    path: `/${path}`,
    url: publicScriptUrl(folder, filename),
    updated: exists,
    sha: result.sha
  };
}
...
export async function handleScriptsList(_body, github) {
  ...
  for (const option of options) {
    const folder = option.folder || downloadFolder(option.id);
    const scripts = items
      .filter((item) => item.project === folder || item.project === option.id)
      .sort((a, b) => a.filename.localeCompare(b.filename));
    if (!scripts.length) continue;
    groups.push({ ...option, scripts });
  }
  return {
    options,
    groups,
...
export async function handleScriptDelete(body, github) {
  const { allowed } = await scriptCatalog(github);
  const project = resolveScriptProject(String((body && body.project) || ""), allowed);
