...
(function (root) {
  ...
  function randomIdSuffix(length) {
  ...
  function newWritingId(date, suffix) {
    return `${isoDate(date)}-${requireSuffix(suffix)}`;
  }

  function newGuideId(date, suffix) {
    return `g-${isoDate(date).replace(/-/g, '')}-${requireSuffix(suffix)}`;
  }

  root.KolTiginSlugify = {
    slugify,
    isValidSlug,
