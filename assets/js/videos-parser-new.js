...
class VideosParser {
    ...
    async loadVideos() {
        try {
            ...
            // Loading state
            if (this.videosList) {
                this.videosList.innerHTML = '<div style="text-align: center; padding: 40px; color: var(--light-gray);">Videolar yükleniyor...</div>';
            }
            ...
            if (this.videos.length === 0) {
                this.showError('Video bulunamadı.');
            }

        } catch (error) {
            console.error('Error loading videos:', error);
            this.showError('Videolar yüklenirken hata oluştu.');
        }
    }
